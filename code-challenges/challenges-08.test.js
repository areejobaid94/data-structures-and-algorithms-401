...
const typeNum = (arr) => {
  // Solution code here...
  return arr.filter(ele => isNaN(ele) == false);
};
...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  return arr.filter(ele => /a|o|u|i|e|A|O|U|I|E/.test(ele));
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  return arr.filter(ele => {
    if (ele.baseStat > minBaseStat) return ele.stat.name;
  }).map(ele => {
