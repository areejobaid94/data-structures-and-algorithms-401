...
const longestString = (arr) => arr.length ? arr.reduce((maxStr,ele,i)=>arr[maxStr].length > ele.length? maxStr : i ,0) : -1;
...
const standardizePhoneNumbers = (arr) => arr.map(ele=> ele.split(/\(|\)|\-|\s/g).join(''));
...
const onlyOddChars = (str) => str.split('').filter((ele,i)=> i%2 !=0).join('');
...
const allHappy = (arr) => arr.reduce((res,ele)=> ele.includes(':)') && res,true);
...
const findEvery = (arr, target) => arr.reduce((res,ele)=> ele.includes(target) && res,true);
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map((ele)=>{
    return ele.reduce((inArr,inEle)=>{
      inEle.includes('Brook') == false ? inArr.push(inEle): inArr;
      return inArr  
    },[])
  }) 
};
...
const characterByIndex = (arr) => arr.map((ele,i)=> ele[i])
