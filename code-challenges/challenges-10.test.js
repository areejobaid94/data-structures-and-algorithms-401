...
const count = (target, input) => {
  // Solution code here...
  return input.flat().reduce((total,ele)=>{
    return ele == target ? total + 1 : total;
  },0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(element => {
    return element.reduce((arr, ele)=>{
      typeof(ele) == 'number' && ele % 5 == 0 ? arr.push(Math.pow(2,ele)) : arr;
      return arr;
    },[]);
  });
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.reduce((str,ele)=>{
    return ele.gender == 'female' || ele.gender == 'male' ? str.length == 0 ? ele.name : `${str} and ${ele.name}`: str;
  },'');
};
...
let findShortest = (data) => {
  ...
  return data.reduce((shortest,ele)=>{
    return shortest.height < ele.height ? shortest = ele : shortest;
  },data[0]).name;
};
